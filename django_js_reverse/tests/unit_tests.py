...
sys.path.insert(
    0, os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "..") + os.sep
)
...
def node_jseval(expr):
    ...
    stdout = subprocess.check_output(["node", "-e", six.ensure_str(module)]).decode(
        "utf8"
    )
    return re.sub(r"\n$", "", stdout)
...
class AbstractJSReverseTestCase(object):
    ...
    def assertEqualJSEval(self, js, url_call, expected_url):
        ...
        self.assertEqual(
            js2py_jseval("(function () {{ {} }}())".format(script)), url(js2py_jseval)
        )
...
class JSReverseViewTestCaseMinified(AbstractJSReverseTestCase, TestCase):
    ...
    def test_view_one_url_arg(self):
        self.assertEqualJSUrlEval(
            'Urls.test_one_url_args("arg_one")', "/test_one_url_args/arg_one/"
        )

    def test_view_two_url_args(self):
        self.assertEqualJSUrlEval(
            'Urls.test_two_url_args("arg_one", "arg_two")',
    ...
    def test_view_optional_url_arg(self):
        ...
        self.assertEqualJSUrlEval(
            'Urls.test_optional_url_arg("arg_one", "arg_two")',
            "/test_optional_url_arg/1_arg_one-2_arg_two/",
        )

    def test_unicode_url_name(self):
        self.assertEqualJSUrlEval(
            "Urls.test_unicode_url_name()", "/test_unicode_url_name/"
        )
    ...
    def test_namespaces(self):
        self.assertEqualJSUrlEval(
            'Urls["ns1:test_two_url_args"]("arg_one", "arg_two")',
            "/ns1/test_two_url_args/arg_one-arg_two/",
        )
        self.assertEqualJSUrlEval(
            'Urls["ns2:test_two_url_args"]("arg_one", "arg_two")',
    ...
    def test_only_namespaces_without_subnss(self):
        response = self.client.get("/jsreverse/")
        self.assertEqualJSUrlEval(
            'Urls["nsno:test_two_url_args"]("arg_one", "arg_two")',
    ...
    def test_duplicate_name(self):
        self.assertEqualJSUrlEval(
            'Urls.test_duplicate_name("arg_one")', "/test_duplicate_name/arg_one/"
        )
        self.assertEqualJSUrlEval(
            'Urls.test_duplicate_name("arg_one", "arg_two")',
            "/test_duplicate_name/arg_one-arg_two/",
        )

    def test_duplicate_argcount(self):
        self.assertEqualJSUrlEval(
            'Urls.test_duplicate_argcount({arg_one: "arg_one"})',
            "/test_duplicate_argcount/arg_one-/",
        )
        self.assertEqualJSUrlEval(
            'Urls.test_duplicate_argcount({arg_two: "arg_two"})',
    ...
    def test_int_args(self):
        self.assertEqualJSUrlEval(
            "Urls.test_two_url_args(0, 5)", "/test_two_url_args/0-5/"
        )
        self.assertEqualJSUrlEval(
            'Urls.test_two_url_args("", 5)', "/test_two_url_args/-5/"
        )

    def test_float_args(self):
        self.assertEqualJSUrlEval(
            "Urls.test_two_url_args(0, 5.5)", "/test_two_url_args/0-5.5/"
        )
    ...
    def test_django_path_syntax(self):
        if is_django_ver_gte_2():
            self.assertEqualJSUrlEval(
                'Urls.test_django_gte_2_path_syntax(42, "foo")',
...
class JSReverseStaticFileSaveTest(AbstractJSReverseTestCase, TestCase):
    def test_reverse_js_file_save(self):
        ...
        path = os.path.join(
            settings.STATIC_ROOT, "django_js_reverse", "js", "reverse.js"
        )
        f = io.open(path)
        content1 = f.read()
        ...
        self.assertEqual(
            len(content1),
            len(content2),
            "Static file don't match http response content_1",
        )
        self.assertEqual(
            content1, content2, "Static file don't match http response content_2"
        )
    ...
    def test_reverse_js_file_save_with_output_path_option(self):
        with mkdtemp(__name__) as js_output_path, override_settings(
            JS_REVERSE_OUTPUT_PATH=js_output_path
        ):
            call_command("collectstatic_js_reverse")
    ...
    def test_script_prefix_noslash(self):
        ...
        with override_settings(JS_REVERSE_SCRIPT_PREFIX=script_prefix):
            self.assertEqualJSUrlEval(
                "Urls.test_no_url_args()", "{0}/test_no_url_args/".format(script_prefix)
            )
...
class JSReverseTemplateTagTest(AbstractJSReverseTestCase, TestCase):
    def test_tpl_tag_with_request_in_context(self):
        ...
        self.assertEqualJSEval(
            js, "Urls.test_changed_urlconf()", "/test_changed_urlconf/"
        )
