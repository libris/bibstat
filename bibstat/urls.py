...
from libstat.views.surveys import (
    surveys,
    surveys_statuses,
    surveys_export,
...
from libstat.views.survey import (
    survey,
    survey_status,
    survey_notes,
    example_survey,
...
from libstat.views.variables import (
    variables,
    edit_variable,
    create_variable,
...
urlpatterns = [
    ...
    re_path(
        ...
        TemplateView.as_view(
            template_name="humans.txt", content_type="text/plain; charset=utf-8"
        ),
        name="humans",
    ),
    # APIs
    ...
    re_path(
        r"^articles/delete/(?P<article_id>\w+)$",
        articles_delete,
    ...
    # Administration
    re_path(
        r"^administration/create_new_collection$",
    ...
    re_path(
        r"^surveys/export_with_previous$",
        surveys_export_with_previous,
        name="surveys_export_with_previous",
    ),
    re_path(
        r"^surveys/import_and_create$",
        import_and_create,
        name="surveys_import_and_create",
    ),
    re_path(r"^surveys/remove_empty$", remove_empty, name="surveys_remove_empty"),
    re_path(
        r"^surveys/match_libraries$", match_libraries, name="surveys_match_libraries"
    ),
    re_path(r"^surveys/status$", surveys_statuses, name="surveys_statuses"),
    re_path(
        r"^surveys/overview/(?P<sample_year>\w+)$",
        surveys_overview,
        name="surveys_overview",
    ),
    re_path(
        r"^surveys/status/(?P<survey_id>\w+)$", survey_status, name="survey_status"
    ),
    re_path(r"^surveys/notes/(?P<survey_id>\w+)$", survey_notes, name="survey_notes"),
    re_path(
        r"^surveys/update_library$",
        surveys_update_library,
        name="surveys_update_library",
    ),
    re_path(r"^surveys/sigel/(?P<sigel>\w+)$", sigel_survey, name="sigel_survey"),
    re_path(
        r"^surveys/unlock/(?P<survey_id>\w+)$",
        release_survey_lock,
        name="release_survey_lock",
    ),
    ...
    re_path(
        r"^variables/replaceable$", replaceable_variables, name="replaceable_variables"
    ),
