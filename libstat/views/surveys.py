...
from libstat.services.clean_data import (
    remove_empty_surveys,
    match_libraries_and_replace_sigel,
)
from libstat.models import Survey, SurveyObservation, Variable
from libstat.services.excel_export import (
    surveys_to_excel_workbook,
    public_excel_workbook,
)
...
def surveys(request, *args, **kwargs):
    ...
    sample_year = request.GET.get(
        "sample_year", str(sample_years[0]) if sample_years else ""
    )
    ...
    exclude_co_reported_by_other = request.GET.get(
...
def _surveys_redirect(request):
    ...
    return HttpResponseRedirect(
        "{}{}".format(
            reverse("surveys"),
            "?action=list&sample_year={}&municipality_code={}&target_group={}&status={}&email={}&free_text={}&surveys_state={}".format(
                sample_year,
                municipality_code,
                target_group,
                status,
...
def _surveys_export_base(request, include_previous_year=False):
    if request.method == "POST":
        survey_ids = request.POST.getlist("survey-response-ids", [])
        filename = "Exporterade enkätsvar ({}).xlsx".format(
        ...
        response = HttpResponse(
            save_virtual_workbook(workbook), content_type="application/vnd.ms-excel"
        )
...
def surveys_overview(request, sample_year):
    ...
    # Preload surveys from database
    all_active_surveys = list(
        Survey.objects.filter(is_active=True, sample_year=sample_year)
    )
    all_active_non_coreported_surveys = list(
        Survey.objects.by(
            is_active=True, sample_year=sample_year, exclude_co_reported_by_other=True
        )
    )  # Using SurveyQuerySet
    ...
    for library_type in utils.SURVEY_TARGET_GROUPS:
        ...
        for status in Survey.STATUSES:
            ...
            surveys = [
                s
                for s in all_active_surveys
                if s._status == status_value
                and s.library.library_type == library_type_value
            ]
            surveys_all_count = len(surveys)
            surveys_non_coreporting = [
                s
                for s in all_active_non_coreported_surveys
                if s._status == status_value
                and s.library.library_type == library_type_value
            ]
            surveys_non_coreporting_count = len(surveys_non_coreporting)
            row.append("%d (%d)" % (surveys_non_coreporting_count, surveys_all_count))

        surveys_total_status = [
            s
            for s in all_active_surveys
            if s.library.library_type == library_type_value
        ]
        surveys_total_status_all_count = len(surveys_total_status)
        surveys_total_status_non_coreporting = [
            s
            for s in all_active_non_coreported_surveys
            if s.library.library_type == library_type_value
        ]
        surveys_total_status_all_non_coreporting_count = len(
            surveys_total_status_non_coreporting
        )
        row.append(
            "%d (%d)"
            % (
                surveys_total_status_all_non_coreporting_count,
                surveys_total_status_all_count,
            )
        )
    ...
    for status in Survey.STATUSES:
        ...
        surveys_total_non_coreporting = [
            s
            for s in all_active_non_coreported_surveys
            if s._status == status_value_str
        ]
        surveys_total_all_non_coreporting_count = len(surveys_total_non_coreporting)
        row.append(
            "%d (%d)"
            % (surveys_total_all_non_coreporting_count, surveys_total_all_count)
        )

    surveys_total_total_all_count = len(all_active_surveys)
    surveys_total_total_all_non_coreporting_count = len(
        all_active_non_coreported_surveys
    )
    row.append(
        "%d (%d)"
        % (surveys_total_total_all_non_coreporting_count, surveys_total_total_all_count)
    )

    table.append(row)

    context = {"sample_year": sample_year, "table": table}
...
def surveys_statuses(request):
    ...
    if status == "published":
        ...
        if num_successful_published != len(survey_response_ids):
            message = (
                "{} Kunde inte publicera {} enkäter eftersom de inte har markerat att "
...
def _create_surveys(libraries, sample_year, ignore_missing_variables=False):
    ...
    for library in libraries:
        existing_surveys = Survey.objects.filter(
            sample_year=sample_year, library__sigel=library.sigel
        ).only("library")
        if existing_surveys.count() != 0:
            survey = existing_surveys[0]
            survey.library = library
        else:
            survey = Survey(library=library, sample_year=sample_year, observations=[])
...
def _update_library_from_json(survey, json_data):
    ...
    if json_data.get("address", None):
        location = next(
            (a for a in json_data["address"] if a["address_type"] == "stat"), None
        )
...
def surveys_update_library(request):
    ...
    if request.method == "POST":
        ...
        if post_data:
            ...
            if sigel:
                existing_surveys = Survey.objects.filter(
                    is_active=True, library__sigel=sigel
                ).only("library")
                if existing_surveys.count() != 0:
                    for survey in existing_surveys:
                        ...
                        survey.save()
                    logger.info(
                        "Bibdb library data change sync: updated survey data for sigel %s"
                        % sigel
                    )
                    return HttpResponse("Updated survey data for sigel %s" % sigel)
                else:
                    logger.info(
                        "Bibdb library data change sync: Tried to update survey data for sigel %s, but couldn't find any active surveys."
                        % sigel
                    )
                    return HttpResponse(
                        "Tried to update survey data for sigel %s, but couldn't find any active surveys."
                        % sigel
                    )
            else:
    ...
    logger.warn(
        "Received illegal request to /surveys/update_library. HTTP-method: %s"
        % request.method
    )
