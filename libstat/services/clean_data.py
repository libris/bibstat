...
def match_libraries_and_replace_sigel(sample_year):
    ...
    f = codecs.open(os.path.join(settings.CLEAN_DATA_LOG_PATH, 'match_libraries_log_%d' % sample_year), 'wt', 'utf-8')
    logfile = File(f)
    ...
    for survey in all_published_surveys:
        if len(survey.library.sigel) == 10 or survey.library.sigel.startswith("8", 0, 1): # Random-sigel or 8*-sigel
            ...
            matching_surveys = Survey.objects.filter(library__name__iexact=survey.library.name, pk__ne=survey.pk)
                ...
                for index, matched_survey in enumerate(matching_surveys):
                    if matched_survey.library.sigel and matched_survey.library.sigel != survey.library.sigel and len(matched_survey.library.sigel) != 10:
                        ...
                        if matched_survey.library.sigel.startswith("8", 0, 1) == False or index == len(matching_surveys) - 1:
                            logfile.write(
                                "Matched %s to %s. Changing sigel %s to %s\n" % (survey.library.name, matched_survey.library.name, survey.library.sigel, matched_survey.library.sigel))
    ...
    for survey_8 in surveys_sigel_start_w_8:
        ...
        if  sigel_from_mapping:
            logfile.write("Found mapping for 8*-sigel %s, (library: %s), changing to %s\n" % (survey_8.library.sigel, survey_8.library.name, sigel_from_mapping))
    ...
    logfile.write("Found %d number of published surveys with random- (or 8*-)sigels\n" % count)
...
def remove_empty_surveys(sample_year, mode):
    ...
    f = codecs.open(os.path.join(settings.CLEAN_DATA_LOG_PATH, 'remove_surveys_log_%d' % sample_year), 'wt', 'utf-8')
    logfile = File(f)
