...
def library_from_json(json_data):
    ...
    library.zip_code = (
        location["zip_code"] if location and location["zip_code"] else None
    )
    ...
    if school_code:
        external_identifier = ExternalIdentifier(
            type="school_code", identifier=school_code
        )
...
def fetch_libraries():
    ...
    for start_index in range(0, 30000, 200):
        response = requests.get(
            url="%s/api/lib?dump=true&start=%d"
            % (settings.BIBDB_BASE_URL, start_index),
            headers={"APIKEY-AUTH-HEADER": "bibstataccess"},
        )  # KP 180110
