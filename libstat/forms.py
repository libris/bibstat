...
class SurveyResponseForm(forms.Form):
    ...
    library_name = forms.CharField(required=True, max_length=100,
                                   widget=forms.TextInput(attrs={'class': 'form-control', 'size': '58'}))
    municipality_name = forms.CharField(required=False, max_length=100,
                                        widget=forms.TextInput(attrs={'class': 'form-control', 'size': '58'}))
    municipality_code = forms.CharField(required=False, max_length=6,
                                        widget=forms.TextInput(attrs={'class': 'form-control width-auto', 'size': '6'}))

    respondent_name = forms.CharField(required=False, max_length=100,
                                      widget=forms.TextInput(attrs={'class': 'form-control', 'size': '58'}))
    respondent_email = forms.EmailField(required=False, max_length=100,
                                        widget=forms.EmailInput(attrs={'class': 'form-control', 'size': '58'}))
    respondent_phone = forms.CharField(required=False, max_length=20,
                                       widget=forms.TextInput(attrs={'class': 'form-control width-auto', 'size': '20'}))
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance:
            ...
            self.fields[
                'municipality_name'].initial = self.instance.metadata.municipality_name if self.instance.metadata else None
            self.fields[
                'municipality_code'].initial = self.instance.metadata.municipality_code if self.instance.metadata else None
            self.fields[
                'respondent_name'].initial = self.instance.metadata.respondent_name if self.instance.metadata else None
            self.fields[
                'respondent_email'].initial = self.instance.metadata.respondent_email if self.instance.metadata else None
            self.fields[
                'respondent_phone'].initial = self.instance.metadata.respondent_phone if self.instance.metadata else None
        else:
        ...
        if self.instance:
            for index, observation in enumerate(self.instance.observations):
                if isinstance(observation.variable.label, list):
                    label = mark_safe(
                        u"<div class='survey-question-order'><span class='survey-index'>{}.</span> <span class='term-key'>{}</span></div><div class='survey-question'>{}</div><div class='survey-question-part'>{}</div>".format(
                            index + 1, observation.variable.key, observation.variable.label[0],
                            observation.variable.label[1]))
                else:
                    label = mark_safe(
                        u"<div class='survey-question-order'><span class='survey-index'>{}.</span> <span class='term-key'>{}</span></div><div class='survey-question'>{}</div>".format(
                ...
                if observation.variable.type == TYPE_BOOLEAN[0]:
                    ...
                    self.fields[observation._source_key] = forms.IntegerField(required=False,
                                                                              ...
                                                                              label=label,
                                                                              initial=int(round(
                                                                                  observation.value)) if observation.value and isinstance(
                                                                                  observation.value,
                                                                                  (int, long, float)) else None)
                    ...
                    self.fields[observation._source_key] = forms.DecimalField(required=False,
                                                                              ...
                                                                              initial=round(observation.value,
                                                                                            2) if observation.value and isinstance(
                                                                                  observation.value,
                                                                                  (int, long, float)) else None)
    ...
    def clean(self):
        ...
        if 'add_survey_question' in cleaned_data and cleaned_data['add_survey_question']:
            ...
            try:
                ...
                self._errors['add_survey_question'] = self.error_class(
                    [u"Termen finns inte eller är inte aktiv. Ange en annan term."])
