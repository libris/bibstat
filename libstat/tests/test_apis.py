...
class OpenDataApiTest(MongoTestCase):
    def setUp(self):
        ...
        d1 = OpenData(library_name=u"NORRBOTTENS LÄNSBIBLIOTEK", library_id=u"81", sample_year=2013,
                      target_group="public", variable=v1, value=7, date_created=creation_date, date_modified=date1)
        d1.save()

        date1 = datetime(2014, 06, 03, 15, 28, 31)
        d1 = OpenData(library_name=u"KARLSTAD STADSBIBLIOTEK", library_id=u"323", sample_year=2013,
                      target_group="public", variable=v1, value=6, date_created=creation_date, date_modified=date1)
        d1.save()

        date2 = datetime(2014, 06, 04, 11, 14, 01)
        d2 = OpenData(library_name=u"GRÄNNA BIBLIOTEK", library_id=u"11070", sample_year=2013, target_group="public",
                      variable=v2, value=1, date_created=creation_date, date_modified=date2)
        d2.save()
    ...
    def test_should_filter_data_by_date_range(self):
        response = self.client.get(
            u"{}?from_date=2014-06-03T15:28:31.000&to_date=2014-06-04T11:14:00.000".format(reverse("data_api")))
        data = json.loads(response.content)
        self.assertEquals(len(data[u"observations"]), 1)
        self.assertEquals(data[u"observations"][0][u"library"][u"@id"],
...
class ObservationApiTest(MongoTestCase):
    def setUp(self):
        ...
        v1.save()
        date1 = datetime(2014, 06, 02, 17, 57, 16)
        obs = OpenData(library_name=u"NORRBOTTENS LÄNSBIBLIOTEK", library_id=u"81", sample_year=2013,
                       target_group="public", variable=v1, value=7, date_created=date1, date_modified=date1)
        obs.save()
...
class TermsApiTest(MongoTestCase):
    def setUp(self):
        v = Variable(key=u"folk5", description=u"Antal bemannade serviceställen, sammanräknat", type="integer",
                     is_public=True, target_groups=["public"])
        v.save()
        v2 = Variable(key=u"Folk69", description=u"Totalt nyförvärv AV-medier", type="integer", is_public=True,
...
class TermApiTest(MongoTestCase):
    def setUp(self):
        ...
        v2 = Variable(key=u"folk6",
                      ...
                      type="integer", is_public=True, target_groups=["public"])
        v2.save()
        v3 = Variable(key=u"Folk69", description=u"Totalt nyförvärv AV-medier", type="integer", is_public=True,
                      target_groups=["public"], is_draft=True)
        v3.save()
