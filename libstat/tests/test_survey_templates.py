...
class TestSurveyTemplate(MongoTestCase):
        ...
        group = Group(
            rows=[
                Row(
                    cells=[
                        Cell(
                            variable_key=self._dummy_variable(
                                key="var1_key", question="sample_question_text"
                            ).key
                        )
                    ]
                )
            ...
            rows=[
                Row(
                    cells=[
                        Cell(
                            variable_key=self._dummy_variable(
                                key="var1_key", category="header1"
                            ).key
                        ),
                        Cell(
                            variable_key=self._dummy_variable(
                                key="var2_key", category="header2"
                            ).key
                        ),
                        Cell(
                            variable_key=self._dummy_variable(
                                key="var3_key", category="header3"
                            ).key
                        ),
                    ]
                )
    ...
    def test_row_has_correct_description(self):
        row = Row(
            cells=[
                Cell(
                    variable_key=self._dummy_variable(
                        key="var1_key", sub_category="row_description1"
                    ).key
                ),
                Cell(variable_key=self._dummy_variable(key="var2_key").key),
                Cell(variable_key=self._dummy_variable(key="var3_key").key),
    ...
    def test_cell_has_correct_explanation(self):
        cell = Cell(
            variable_key=self._dummy_variable(
                key="var1_key", description="var1_sample_description"
            ).key
        )
