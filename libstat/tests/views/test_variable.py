...
class VariablesViewTest(MongoTestCase):
    ...
    def test_should_filter_variables_by_list_of_target_groups(self):
        ...
        response = self.client.get(
            "{}?target_group=skolbib&target_group=folkbib".format(self.url)
        )
    ...
    def test_each_variable_should_have_edit_link(self):
        ...
        self.assertContains(
            ...
            (
                ...
                'href="#" class="edit-variable">key_1</a>'
            ).format(variable.id),
            count=1,
            status_code=200,
            html=True,
    ...
    def test_should_have_button_for_adding_variable(self):
        ...
        self.assertContains(
            ...
            (
                ...
                'href="#" data-form="{}">Skapa term</a>'
            ).format(reverse("create_variable")),
            count=1,
            status_code=200,
            html=True,
        )
...
class CreateVariableViewTest(MongoTestCase):
    ...
    def test_should_create_variable_draft(self):
        response = self.client.post(
            self.url,
            {
                ...
                "question": (
                    "Hur många anställda personer fanns i biblioteksverksamheten den "
                ...
                "description": (
                    "Antal anställda manliga bibliotekarier den "
                    "1 mars aktuellt mätår"
                ),
        ...
        self.assertEqual(
            result.question_part, "Antal anställda bibliotekarier som är män"
        )
        ...
        self.assertEqual(
            result.target_groups, ["folkbib", "specbib", "skolbib", "sjukbib"]
        )
        self.assertEqual(
            result.description,
            "Antal anställda manliga bibliotekarier den 1 mars aktuellt mätår",
        )
    ...
    def test_should_return_validation_errors_when_omitting_mandatory_fields(self):
        ...
        self.assertEqual(
            data["errors"]["target_groups"], ["Detta fält måste fyllas i."]
        )
...
class EditVariableViewTest(MongoTestCase):
    def setUp(self):
        self.v1 = Variable(
            key="Folk10",
            description="Antal bemannade serviceställen",
            type="integer",
            is_public=True,
        ...
        self.v2 = Variable(
            ...
            type="integer",
            is_public=True,
            target_groups=["sjukbib"],
        )
        self.v2.save()
        self.v3 = Variable(
            key="Forsk23",
            description="Antal anställda personer manliga biblioteksassistenter.",
            type="integer",
            is_public=True,
            is_draft=True,
            target_groups=["specbib"],
        )
        self.v3.save()
    ...
    def test_draft_should_replace_variable(self):
        response = self.client.post(
            reverse("edit_variable", kwargs={"variable_id": str(self.v3.id)}),
            {
                "active_from": "2015-01-01",
                "type": self.new_type,
                "target_groups": self.new_target_groups,
    ...
    def test_should_not_be_able_to_edit_active_to_for_replaced(self):
        ...
        self.v2.replace_siblings(
            [self.v1.id], switchover_date=datetime(2015, 1, 1).date(), commit=True
        )
        ...
        self.assertContains(
            response,
            (
                '<input type="text" disabled="disabled" value="2015-01-01" name="active_to"'
        ...
        response = self.client.post(
            ...
            {
                "type": self.v1.type,
                "target_groups": self.v1.target_groups,
    ...
    def test_should_be_able_to_activate_draft(self):
        edit_draft_url = reverse(
            "edit_variable", kwargs={"variable_id": str(self.v3.id)}
        )
        ...
        self.assertContains(
            response,
            (
                '<input type="submit" id="save_and_activate" value="Spara och aktivera"'
    ...
    def test_activating_draft_with_existing_replacements_should_replaces_siblings(self):
        ...
        self.v3.replace_siblings(
            [self.v2.id], switchover_date=datetime(2015, 1, 1).date(), commit=True
        )
    ...
    def test_should_keep_active_to_date_when_updating_replaced_variable(self):
        ...
        replaced_by = self.new_variable(
            key="antalManligaBibliotekarier", is_draft=False
        )
        replaced_by.active_from = datetime(2015, 1, 1)
        replaced_by.replace_siblings(
            [replaced.id], switchover_date=replaced_by.active_from, commit=True
        )
    ...
    def test_should_not_be_able_to_delete_non_draft_variable_when_referenced_in_survey_response(
        self,
    ):
    ...
    def test_should_not_be_able_to_delete_non_draft_variable_when_referenced_in_open_data(
        self,
    ):
    ...
    def test_should_remove_reference_in_replaced_variables_when_deleting_non_draft_variable(
        self,
    ):
        replaced = self.new_variable()
        replacement = self.new_variable(is_draft=False)
        replacement.active_from = datetime(2015, 1, 1)
        replacement.replace_siblings(
            [replaced.id], switchover_date=replacement.active_from, commit=True
        )
    ...
    def test_should_remove_reference_in_replacement_variable_when_deleting_non_draft_variable(
        self,
    ):
    ...
    def test_should_remove_reference_in_replaced_variables_when_deleting_draft_variable(
        self,
    ):
        replaced = self.new_variable()
        replacement = self.new_variable()
    ...
    def test_should_remove_reference_in_replacement_variable_when_deleting_draft_variable(
        self,
    ):
        replaced = self.new_variable()
        replacement = self.new_variable()
    ...
    def post(self, action, variable):
        ...
        return self.client.post(
            url,
            {
                "active_from": variable.active_from.date()
                if variable.active_from
                else "",
                # active_to input is disabled if variable is replaced
                "active_to": variable.active_to.date()
                if variable.active_to and not variable.replaced_by
                else "",
                "replaces": ", ".join([str(v.id) for v in variable.replaces])
    ...
    def new_survey_response(self):
        return Survey(target_group="folkbib", library_name="test", sample_year="1234")
    ...
    def assertDoesNotExist(self, variable):
        self.assertRaises(
            Variable.DoesNotExist, lambda: Variable.objects.get(pk=variable.id)
        )
...
class ReplaceableVariablesApiTest(MongoTestCase):
    ...
    def test_should_return_replaceable_variables_as_json(self):
        ...
        self.assertEqual(
            data,
            [
                {"key": "key_1", "id": str(var1.id), "description": var1.description},
                {"key": "key_4", "id": str(var4.id), "description": var4.description},
            ],
        )
    ...
    def test_should_filter_replaceables_by_key(self):
        ...
        data = json.loads(response.content)
        self.assertEqual(
            data, [{"key": "Folk28", "id": str(var.id), "description": var.description}]
        )
    ...
    def test_should_filter_replaceables_by_description(self):
        ...
        data = json.loads(response.content)
        self.assertEqual(
            data, [{"key": "Skol10", "id": str(var.id), "description": var.description}]
        )
