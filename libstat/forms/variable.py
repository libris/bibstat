...
class VariableForm(forms.Form):
    ...
    is_public = forms.BooleanField(
        required=False, widget=forms.CheckboxInput(attrs={"value": "1"})
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["target_groups"].choices = [
            target_group for target_group in SURVEY_TARGET_GROUPS
        ]

        if self.instance:
            self.fields["active_from"].initial = (
                self.instance.active_from.date() if self.instance.active_from else None
            )
            self.fields["active_to"].initial = (
                self.instance.active_to.date() if self.instance.active_to else None
            )
            ...
            self.fields["replaces"].initial = (
                ", ".join([str(v.id) for v in self.instance.replaces])
                if self.instance.replaces
                else ""
            )
            self.replaces_initial_value = ", ".join(
                ["{}:{}".format(v.key, str(v.id)) for v in self.instance.replaces]
                if self.instance.replaces
                else []
            )
    ...
    def clean(self):
        ...
        active_from = (
            cleaned_data["active_from"] if "active_from" in cleaned_data else None
        )
        ...
        if (
            self.instance
            and self.instance.replaced_by
            and active_to
            and self.instance.active_to
    ...
    def save(self, commit=True, user=None, activate=False):
        ...
        variable.active_from = self.cleaned_data[
            "active_from"
        ]  # Need to convert to UTC? It's a date and not a datetime...
        variable.active_to = (
            self.instance.active_to
            if self.instance and self.instance.replaced_by
            else self.cleaned_data["active_to"]
        )
        ...
        to_replace = (
            self.cleaned_data["replaces"].split(", ")
            if self.cleaned_data["replaces"]
            else []
        )
        modified_siblings = variable.replace_siblings(
            to_replace, switchover_date=variable.active_from
        )
