...
class SurveyForm(forms.Form):
    def _cell_to_input_field(self, cell, observation, authenticated, variable_type):
        ...
        if cell.part_of:
            ...
            if isinstance(cell.part_of, list):
                ...
                ] = "Värdet får inte vara högre än värdet i följande fält: {}".format(
                    part_of_as_string
                )
            else:
                ...
                attrs[
                    "data-bv-callback-message"
                ] = "Värdet får inte vara högre än värdet i fältet {}".format(
                    cell.part_of
                )
        ...
        if cell.has_part:
            ...
            if isinstance(cell.has_part, list):
                ...
                if not cell.part_of:
                    attrs[
                        "data-bv-callback-message"
                    ] = "Värdet får inte vara mindre än värdet i följande fält: {}".format(
                        has_part_as_string
                    )
            else:
                ...
                if not cell.part_of:
                    attrs[
                        "data-bv-callback-message"
                    ] = "Värdet får inte vara mindre än värdet i fältet {}".format(
                        cell.has_part
                    )
        ...
        if "Utgift" in cell.variable_key or "Intakt" in cell.variable_key:
            ...
            if variable_type == "integer":
                ...
                ] = "Vänligen mata in ett numeriskt värde mindre än eller lika med 999 999 999, alternativt '-' om värdet inte är relevant"
            elif variable_type == "decimal":
                attrs["data-bv-regexp-regexp"] = "^(-|[\d ]{1,11}(\,[\d ]{1,3})?)$"
                attrs[
                    "data-bv-regexp-message"
                ] = "Vänligen mata in ett numeriskt värde mindre än eller lika med 999 999 999,999 med max 3 decimaler (t ex 12,522), alternativt '-' om värdet inte är relevant"
            ...
            if variable_type == "decimal":
                ...
                attrs[
                    "data-bv-regexp-message"
                ] = "Vänligen mata in ett numeriskt värde mindre än eller lika med 99999999,999 med max 3 decimaler (t ex 12,522), alternativt '-' om värdet inte är relevant"
            ...
            if variable_type == "email":
                ...
                attrs[
                    "data-bv-regexp-message"
                ] = "Vänligen mata in en giltig emailadress"
            ...
            if variable_type == "phonenumber":
                ...
                attrs[
                    "data-bv-regexp-message"
                ] = "Vänligen mata in ett giltigt telefonnummer utan bokstäver och parenteser, t ex 010-709 30 00"
        ...
        if not observation or observation.value_unknown:
            ...
            attrs["data-original-value"] = (
                observation.value
                if observation and observation.value is not None
                else ""
            )
        ...
        if not observation or observation.value_unknown:
            ...
            field.initial = str(observation.value).replace(
                ".", ","
            )  # decimals are displayed with comma in the form
        else:
    ...
    def _set_libraries(
        ...
        other_surveys_selected_sigels = current_survey.selected_sigels_in_other_surveys(
            self.sample_year
        )
        ...
        def set_library(self, library, current_library=False):
            ...
            attrs = {"value": checkbox_id, "class": "select-library"}
            ...
            if library.sigel in other_surveys_selected_sigels:
                ...
                row[
                    "comment"
                ] = "Detta bibliotek rapporteras redan för i en annan enkät."
                if current_library or library.sigel in this_surveys_selected_sigels:
                    row[
                        "comment"
                    ] = "Rapporteringen för detta bibliotek kolliderar med en annan enkät."
            ...
            if current_library:
                ...
                if not library.sigel in other_surveys_selected_sigels:
                    row[
                        "comment"
                    ] = "Detta är det bibliotek som mottagit denna enkät. Om du samredovisar med andra bibliotek, glöm inte att även kryssa för dem här i listan."
    ...
    def _mailto_link(self):
        body = (
            "%0D%0A"
            "----------" + "%0D%0A"
            "Var%20vänlig%20och%20låt%20följande%20information%20stå%20kvar%20i%20meddelandet."
            + "%0D%0A"
            "" + "%0D%0A"
            "Bibliotek:%20{}%20({})%20i%20{}".format(
                self.library_name, self.library_sigel, self.city
            )
            + "%0D%0A"
            "Kommun/län:%20{}%20({})".format(
                municipalities.get(self.municipality_code, ""), self.municipality_code
            )
            + "%0D%0A"
        ...
        return (
            ...
            "?subject=Fråga%20för%20statistikenkät:%20{}%20({})".format(
                self.library_name, self.library_sigel
            )
            + "&body={}".format(body)
        )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["read_only"] = forms.CharField(
            required=False, widget=forms.HiddenInput(attrs={"id": "read_only"})
        )
        self.fields["key"] = forms.CharField(
            required=False, widget=forms.HiddenInput(), initial=survey.pk
        )
        ...
        intro_text = (
            variables[template.intro_text_variable_key].description
            if template.intro_text_variable_key in variables
            else ""
        )
        ...
        self.can_submit = not authenticated and survey.status in (
            "not_viewed",
            "initiated",
        )
        ...
        if (
            hasattr(self, "library_selection_conflict")
